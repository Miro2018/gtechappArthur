...
//            entries = FileUtils.loadEntriesFromAssets(getActivity().getAssets(), "stacked_bars.txt");
     ...
     * @return
     */
    protected PieData generatePieData(List<ContagemRegistros> listaDados, String NomGrafico) {
        
        int count = listaDados.size();
        
        ArrayList<PieEntry> entries1 = new ArrayList<PieEntry>();
        
        for(int i = 0; i < count; i++) {
            entries1.add(new PieEntry((float) listaDados.get(i).getQtdRegistros(), listaDados.get(i).getNomTitulo()));
        }
